...
class UnitTestWriterAgent(BaseAgent):
    ...
    async def execute(self, state: TestGenState) -> TestGenState:
        ...
        planned_ids = {item.symbol_id for item in state.test_plan if item.test_type == "unit"}
        symbols = [symbol for symbol in repo.target_symbols if symbol.symbol_id in planned_ids and not symbol.is_api_route]
        content = await self._render_with_llm(state, symbols)
        if content is None:
            content = self._render(str(repo.repo_path), compact_symbol_rows(symbols))
        path = write_generated_file(state, "test_unit_generated.py", content)
        state.generated_unit_tests.append(path)
        return state
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "你是资深 Python 测试工程师。请生成可运行 pytest 文件。"
                    "只输出 Python 代码，不要解释。不要测试私有实现，不要修改业务代码。"
                ),
            },
            {
                "role": "user",
                "content": (
                    "为下面符号生成保守但有价值的单元测试。要求：\n"
                    "1. 文件必须可直接由 pytest 执行。\n"
                    "2. 必须包含 repo/src 路径注入。\n"
                    "3. 不确定业务输出时，优先验证 import、callable、签名、类定义等稳定行为。\n"
                    "4. 需要 import pytest。\n\n"
...
def test_no_unit_targets_discovered():
    pytest.skip("No ordinary Python functions or classes were discovered.")
'''
        return header(repo_path) + f'''
SYMBOLS = {rows!r}


@pytest.mark.parametrize("module_name,symbol_name,kind,signature", SYMBOLS)
def test_generated_symbol_is_importable(module_name, symbol_name, kind, signature):
